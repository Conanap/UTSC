...
{ 
    ...
    while(1) {
        ...
        if(nready <0) {
            ...
            nread = read(scokfd, &server_counter,  sizeof(int));
            if(nread > 0) {
                printf("Counter is %d\n", server_counter);
            } else if(nread == -1) {
                perror("read");
                ...
                fprintf(stderr, "Server connection is terminated\n");
                Close(sockfd);
                exit(0);
            }
        } else (FD_ISSET(fileno(stdin), &rset)) {
            if(fgets(buf, BUF_SIZE, stdin) == NULL) {
                // error
                exit(1);
            }
            int counter = atoi(buf);
            int nwrite = write(sockfd, &counter, sizeof(int));
