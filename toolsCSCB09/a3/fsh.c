...
int main(int argc, char **argv)
{
    ...
    extern void print_execute(char *cmd);
    int optind;
    FILE *file;
    ...
    if(optind + 1 < argc) { // too many args / -c command + file
        fprintf(stderr, "%s: [-v] [-i] [{file | -c command}]\n", argv[0]);
        return(1);
    } else if(optc) {
        print_execute(argv[optind]);
        return(0);
    }
    else if(optind + 1 == argc) { // file
...
void print_execute(char *cmd) {
    struct parsed_line *p;
    extern void execute(struct parsed_line *p);
    extern void print_command(char *command);
...
void execute_one_subcommand(struct parsed_line *p)
{
    ...
    if (p->pl) {
    	if(!strchr(arg_loc[0], '/')) { // finding right path
    		...
    		loc = arg_loc[0];
    	if(i < 4) { // prevent array out of index (ie runs only if command found)
