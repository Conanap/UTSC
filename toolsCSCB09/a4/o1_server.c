        ...
        while(1) {
                switch(select(maxfd + 1, &fdlist, NULL, NULL, &tv)) {
                        ...
                        if(FD_ISSET(listenfd, &fdlist)) { // new connection
                                ...
                                if(clientfd > maxfd)
                                        maxfd = clientfd;
                                newclient(clientfd, &q); //note: client list is the newest client
                                // supposed to send to client
                                ...
                                for(p = clientlist; p; p = p->next) {
                                        clientfd = p->fd;
                                if(!read(clientfd, p->buf, sizeof p->buf))
                                        removeclient(p);
                                else {
                                ...
                                while((s=memnewline(p->buf, sizeof p->buf)) == NULL) {
                                        ...
                                        do_something(p, s); // p = struct client, s = ret of memnewline
                                }
                                }
                        }
                } // end of switch
