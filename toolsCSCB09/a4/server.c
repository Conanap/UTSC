...
{
        parseargs(argc, argv);
        extern void client_action();
        extern void makeclient();
...
void makeclient() {
        ...
        fd = accept(listenfd, (struct sockaddr *)&q, &len);
        if(fd < 0) {
                perror("accept");
                exit(1);
        }
...
void client_action() {
        ...
        // check ea client for action
        for(p = clientlist; p; p = p->next) {
                        clientfd = p->fd;
                if(FD_ISSET(clientfd, &fdlist) && 
                        !read(clientfd, p->buf, sizeof p->buf))
                        removeclient(p);
                else if(FD_ISSET(clientfd, &fdlist)) {
                        p->bytes_in_buf = sizeof p->buf;
                        while((s=memnewline(p->buf, sizeof p->buf)) == NULL) {
                                read(clientfd, p->buf, sizeof p->buf);
                                p->bytes_in_buf = sizeof p->buf;
                        }
                        do_something(p, s);
...
static void do_say(struct client *p) {
        ...
        struct client *p2;
        char *buf;
        if((buf = malloc(sizeof p->buf + 5)) == NULL) {
                perror("malloc");
                exit(1);
        }
        sprintf(buf, "said %d%s\r\n", id, &(p->buf[4]));
        for(p2 = clientlist; p2; p2 = p2->next) 
                send_string(p2->fd, buf);
}
