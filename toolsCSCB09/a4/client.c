...
static void say(char *buf);
static void said(char *buf);
static void help();
...
{
        if((say_with_space = malloc(sizeof(char) * 5)) == NULL) {
                perror("malloc");
                exit(1);
        }
        // default say line
        sprintf(say_with_space, "%s ", lang_say);
        startup_checks();
        parseargs(argc, argv);
        connect_to_server();
        gethandle();
...
static void do_something()
{
        ...
        if (select(serverfd+1, &fdlist, NULL, NULL, NULL) < 0) {
                ...
                if (FD_ISSET(0, &fdlist)) {
                        ...
                        exit(0);
                        if(!strncmp(say_with_space, buf, strlen(lang_say) + 1)) {
                                say(buf);
                        } else {
                           docmd(explode(buf));
                        }
...
static void say(char *buf) {
        char *newbuf;
        if((newbuf = malloc(sizeof buf)) == NULL) {
                perror("malloc");
                exit(1);
        }
        sprintf(newbuf, "say %s", &buf[strlen(lang_say)]);
        send_string(newbuf);
}
...
static void said(char *buf) {
        char *nbuf, *saybuf, *p;
        int endofnum = 0, id, i;
        while(buf[endofnum] && buf[endofnum] != ' ')
                endofnum++;
        if((nbuf = malloc(sizeof(char)*endofnum)) == NULL) {
                perror("malloc");
                exit(1);
        }
        for(i = 0; i < endofnum; i++)
                nbuf[i] = buf[i];
        nbuf[i] = '\0';
        id = atoi(nbuf);
        saybuf = &buf[endofnum];
